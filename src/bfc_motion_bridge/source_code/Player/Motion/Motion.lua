...
if sit_disable==0 then --For smaller robots
  ...
  --tambahan
--  sm:set_transition(stance, 'fall', falling);
...
--tambahan
  fallCheckState = standup.update();
end

function update()
  -- update us
  UltraSound.update();
  local BodyAngle = walk.bodyTilt * 180/math.pi;
  local imuAngle = Body.get_sensor_imuAngle();
  --local maxImuAngle = math.max(math.abs(imuAngle[1]), math.abs(imuAngle[2]-bodyTilt));
  --local maxSudutImu = math.max(math.abs(imuAngle[1]*180/math.pi), (math.abs(imuAngle[2]*180/math.pi) - (bodyTilt*180/math.pi)));
  --local sudutJatuh = fallAngle*180/math.pi;
  local imuAngleRad = imuAngle;
  local imuAngleX = imuAngleRad[2] - BodyAngle;
  local imuAngleY = imuAngleRad[1];
  local AngleFallFront = 35;
  local AngleFallBack = -50;
  local AngleFallRight = 40;
  local AngleFallLeft = -40;
  local headP = vector.new({0,-120})*math.pi/180;
--  fallCheckState = standup.update();
--  fallCheckState = 'done';
--  fall = mcm.get_motion_fall_check() --Should we check for fall? 1 = yes
  
  --print("imu X \t",imuAngleX);
  --print("imu Y \t",imuAngleY);
  --print("");

  if (imuAngleX > AngleFallFront) then
    --print("DEPAN")
    Body.set_head_command(headP);
    fall = 1;
    sm:add_event("fall");
    mcm.set_walk_isFallDown(0); --Notify world to reset heading 
  elseif (imuAngleX < AngleFallBack) then
  --print("BELAKANG")
    Body.set_head_command(headP);
    fall = 1;
    sm:add_event("fall");
    mcm.set_walk_isFallDown(0); --Notify world to reset heading 
  elseif (imuAngleY > AngleFallRight) then
  --print("KANAN")
    Body.set_head_command(headP);
    fall = 1;
    sm:add_event("fall");
    mcm.set_walk_isFallDown(0); --Notify world to reset heading
  elseif (imuAngleY < AngleFallLeft) then
  --print("KIRI")
    Body.set_head_command(headP);
    fall = 1;
    sm:add_event("fall");
