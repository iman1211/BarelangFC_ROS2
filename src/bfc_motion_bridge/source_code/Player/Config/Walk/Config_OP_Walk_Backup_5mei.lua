...
-- Stance parameters
---------------------------------------------
walk.bodyHeight 	= 0.295; --0.295
walk.bodyTilt		= 20*math.pi/180; 
walk.footX		= -0.035; --20 
walk.footY 		= 0.035;
walk.supportX 		= 0;
walk.supportY 		= 0; --0.010;

walk.qLArm=math.pi/180*vector.new({90,15,-40});
walk.qRArm=math.pi/180*vector.new({90,-15,-40});

walk.qLArmKick=math.pi/180*vector.new({90,30,-60});
walk.qRArmKick=math.pi/180*vector.new({90,-30,-60});

walk.hardnessSupport 	=  1; --.80
walk.hardnessSwing 	= .5; --.5 --.48
walk.hardnessArm 	= .2; --.2 --.35
...
-- Gait parameters
---------------------------------------------
walk.tStep 	= 0.29; --30 --0.27
walk.tZmp 	= 0.130; --0.185
walk.stepHeight = 0.045; --55 --0.035
...
if Config.servo.pid == 1 then
  ...
  --ENABLE Y BALANCING
--  walk.armImuParamY	= {0.25, 35*gyroFactor, 20*math.pi/180, 45*math.pi/180};
  walk.armImuParamY	= {0.3, 100*gyroFactor, 20*math.pi/180, 45*math.pi/180}; --60*gyroFactor

  --DISABLE Y BALANCING
--  walk.armImuParamY	= {0.0, 0*gyroFactor, 20*math.pi/180, 45*math.pi/180};

  walk.kneeImuParamX	= {0.5, 1.2*gyroFactor, 1*math.pi/180, 25*math.pi/180};
  ...
  walk.armImuParamX	= {0.3, 10*gyroFactor, 20*math.pi/180, 45*math.pi/180};
  walk.armImuParamY	= {0.3, 100*gyroFactor, 20*math.pi/180, 45*math.pi/180};
...
walk.supportBack 	= -0.003; --Lean back when walking backward -0.02
