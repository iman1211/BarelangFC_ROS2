...
walk.velLimitX		= {-.03,.08}; --reduced speed for stability
...
-- Stance parameters
---------------------------------------------
walk.bodyHeight 	= 0.455; --0.295 --0.455
walk.bodyTilt		= 15*math.pi/180;--17.5 --15 
walk.footX		= -0.030; --35 -- 20 --30 
walk.footY 		= 0.040; --0.035
walk.supportX 		= 0.0065;--0.0065
walk.supportY 		= 0;-- -0.05; --0.010;

walk.qLArm=math.pi/180*vector.new({107,60,-30}); --15
walk.qRArm=math.pi/180*vector.new({107,-60,-30});  -- -15

walk.qLArmKick=math.pi/180*vector.new({90,65,-60}); --35 --30
walk.qRArmKick=math.pi/180*vector.new({90,-65,-60}); -- -30

walk.hardnessSupport = .75;--.75 --.8
walk.hardnessSwing 	= .60;--.55 --.5
walk.hardnessArm 	= .35;--.35; --.3
...
-- Gait parameters
---------------------------------------------
walk.tStep 	= 0.28 --0.27 --cepat langkah
walk.tZmp 	= 0.120; --0.155; --0.185 --150
walk.stepHeight = 0.08;--0.035 --0.06 --0.055 --0.035 --tinggi langkah
walk.phSingle 	= {0.01,0.9};--0.1,0.9
...
if Config.servo.pid == 1 then
  walk.ankleImuParamX	= {0.5,0.3*gyroFactor, 1*math.pi/180, 25*math.pi/180};
  walk.ankleImuParamY	= {0.5,0.7*gyroFactor, 1*math.pi/180, 25*math.pi/180};
  walk.armImuParamX	= {0.175, 7.5*gyroFactor, 20*math.pi/180, 45*math.pi/180};
--ENABLE Y BALANCING
  walk.armImuParamY	= {0.125, 7.5*gyroFactor, 20*math.pi/180, 45*math.pi/180};
--DISABLE Y BALANCING
--  walk.armImuParamY	= {0, 0*gyroFactor, 20*math.pi/180, 45*math.pi/180};
  walk.kneeImuParamX	= {0,0*gyroFactor, 1*math.pi/180, 25*math.pi/180};
  walk.hipImuParamY	= {0.5,0.3*gyroFactor, 1*math.pi/180, 25*math.pi/180};
...
walk.velFastForward	= 0.05;--0.05
walk.velFastTurn 	= 0.15;--0.15

--walk.supportFront 	= 0.01; --Lean back when walking fast forward
walk.supportFront 	= 0;-- 0 --.03; --Lean back when walking fast forward
walk.supportFront2 	= 0;-- 0 --Lean front when accelerating forward

--walk.supportSideX 	= -0.01; --Lean back when sidestepping
--walk.supportSideX 	= -0.005; --Lean back when sidestepping
walk.supportSideX       =  -0.005;-- 0 --Lean back when sidestepping
--walk.supportSideY 	=  0.02; --Lean sideways when sidestepping
walk.supportSideY       =  0.02;-- 0 --Lean sideways when sidestepping


walk.supportBack 	= 0.015;--0.27 --Lean back when walking backward -0.0015 -0.02  -0.003
walk.supportTurn 	= 0.015;--0.12 --Lean front when turning 0.02
--walk.supportBack 	= 0.035; --Lean back when walking backward -0.0015 -0.02  -0.003
--walk.supportTurn 	= 0.012; --Lean front when turning 0.02
...
walk.walkKickDef["FrontLeft"]={
  {0.30, 1, 0, 0.07 , {0,0}, 0.6, {0.12,0,0} },
  {0.40, 2, 1, 0.115 , {0.04,-0.02}, 0.5, {0.17,0,0}, {0.12,0,0} },
  {0.30, 1, 0, 0.07 , {0.04,0}, 0.5, {0.03,0,0} },
}
walk.walkKickDef["FrontRight"]={
  {0.30, 1, 1, 0.07 , {0,0}, 0.6, {0.12,0,0} },
  {0.40, 2, 0, 0.115 , {0.04,0.02}, 0.5,  {0.17,0,0}, {0.12,0,0} },
  {0.30, 1, 1, 0.07 , {0.04,0}, 0.5, {0.03,0,0} },
}
--[[
walk.walkKickDef["FrontLeft"]={
