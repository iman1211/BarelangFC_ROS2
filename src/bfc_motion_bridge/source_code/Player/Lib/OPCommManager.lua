...
function imu_rpy(id,address,len)
  ...
  local data = Dynamixel.read_data(id,address,len);
  local dataFix = DynamixelPacket.byte_to_word(data[1],data[2])
  local b = string.format("%X", getLowByte16(dataFix))
  -- print(dataFix)
...
--[[   Roll = imu_rpy(200,44,4) * -1
  ...
  --[[ local strategy = Dynamixel.read_data(200,51,4)
